...
int main(void)
{
  ...
  while (1)
  {
	...
	//update CCR value from 0 to calculated value
...
void EXTI0_1_IRQHandler(void)
{
	// TODO: Add code to switch LED7 delay frequency
	 if (HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) == GPIO_PIN_SET) {
		         ...
		         // Toggle between 1 Hz and 2Hz
		         if (delay_t == 500) {
		             // If current delay is 500 msecond, change to 1s
		             delay_t=1000;
		         } else {
		             // If current delay is 1 second, change to 1 second(1000ms)
		             delay_t=500;
		         }
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	//adc_val * ARR  over the max adc value
