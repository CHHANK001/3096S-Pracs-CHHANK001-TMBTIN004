...
int main(void)
{
  ...
  for (uint16_t i = 0; i < sizeof(patterns) / sizeof(patterns[0]); i++) {
      write_to_address(start + i, patterns[i]);
      HAL_Delay(10);
  }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) != GPIO_PIN_SET) {
	         delay(del); // Debounce delay

	         // Toggle between 1-second and half-second delay
	         if (__HAL_TIM_GET_AUTORELOAD(&htim16) == (1000 - 1)) {
	             // If current delay is 1 second, change to half-second
	             __HAL_TIM_SET_AUTORELOAD(&htim16, 500 - 1);
	         } else {
	             // If current delay is half-second, change to 1 second
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t read_value = read_from_address(start+index);
	if (read_value != patterns[index])
		{
